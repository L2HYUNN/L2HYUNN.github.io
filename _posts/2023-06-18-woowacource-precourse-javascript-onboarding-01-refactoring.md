...
## 리팩토링
1번 문제 풀이 이후 기존 풀이에 몇 가지 아쉬운 부분이 있어 리팩토링을 해야겠다고 생각하고 있었다.
...
문제를 풀면서 함수를 가능한 작게 만들려고 노력하였고 가독성을 고려하여 최대한 함수의 동작에 알맞은 이름을 붙이기 위해 노력하였다. 이 과정을 통해 만들어진 함수는 주석없이도 그 동작을 충분히 설명해주고 있었다.

위와 같은 생각을 바탕으로 기존 코드에 있던 주석을 모두 제거하였다. 

### 함수가 하나의 기능만 하는가?
처음 문제를 풀 때도 가장 오래 고민했던 문제이다. 

당시에는 충분한 고민 끝에 가능한 작게 나눴다고 생각했지만 사실 몇몇 함수는 더 작게 나뉠 수 있었다. 작게 나누는 과정에서 `이렇게까지 작게 만들어야 할까?` 라는 고민을 했던 것 같다.

하지만 함수는 하나의 기능만 할 때 그리고 작으면 작을수록 좋다. 그렇기에 가능한 더욱 작게 나누어보기로 했다. 이전 코드를 살펴보며 함수를 더욱 작게 만들어보자.
...
`getMaxNumber`의 동작은 숫자 배열을 받아 `Math.max()`를 통해 최대값을 얻는 함수이다. 이것은 기존 `Math.max()`라는 함수를 이름만 바꿔준 것과 동일하다. 이러한 경우에는 함수로 만들기 보다 있는 그대로 사용하는 것이 오히려 가독성을 높일 수 있는 방법일 것이다.
...
문제의 프로그램은 `pobi`와 `crong`중 승자를 정하는 프로그램이기 때문에 다음과 만들 수 있다.
